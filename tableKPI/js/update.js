...
$("#btnOpenModalEdit").click(function(){
    ...
    $.ajax({
        ...
        error: function(error) {
            alert('Error fetching KPI list data from the databaseeeeeeee.');
        }
    });
        ...
        success: function(data) {
            ...
            table3 = new Tabulator("#table-edit-kpi", {
                ...
                columns: [
                    ...
                    {title: "OBJECTIVE", field: "c",  width:300, formatter:formatterEditKPI, headerSort:false, },
                    {title: "KEY RESULT", field: "f", headerSort:false, formatter:formatterEditGoal,},
...
function formatterEditLastScore(cell, formatterParams, onRendered){
    var input = document.createElement("input");
    var rowData = cell.getRow().getData();
    var value = cell.getValue();
    ...
    input.classList.add("txtLastScore");
    // input.classList.add("txtNumberOnly");

    // Set up an event listener to capture the input value when changed
    input.addEventListener("input", function (event) {
        cell.getRow().update({ lastS: value });
        
    });
    return input;
}
function formatterEditLastGrade(cell, formatterParams, onRendered){
    var input = document.createElement("input");
    var rowData = cell.getRow().getData();
    var value = cell.getValue();
    ...
    input.value = value;
    input.classList.add("form-control");
    input.classList.add("txtLastGrade");
    // input.classList.add("txtNumberOnly");

    // Set up an event listener to capture the input value when changed
    input.addEventListener("input", function (event) {
        cell.getRow().update({ lastG: value });
...
function updateKpiList(){
    ...
    for (var i = 0; i < rows.length; i++){
        ...
        var lastScore = rows[i].querySelector(".txtLastScore").value;
        var lastGrade = rows[i].querySelector(".txtLastGrade").value;
